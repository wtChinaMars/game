...
cc.Class({
    ...
    onLoad: function () {
       ...
       this.addComponent("sjdouzizhu_Room");
    ...
    initView:function(){
        ...
        this._timecount = gameChild.getChildByName('timecount').getComponent(cc.Label);
...
tFun_chupai:function(event){
    ...
    initEventHandlers:function(){
        ...
        this.node.on('game_num',function(data){
             self._gamecount.string = "" + data.detail.numOfGames + "/3局";
          //   self._speedtime=data.detail.speedtime;
 ...
 //分享朋友圈
    ...
    update: function (dt) {
        ...
        }console.log(cc.vv.gameNetMgr.speedTime-Date.now());
        if(cc.vv.gameNetMgr.speedTime-Date.now()>0){
           
            var daojishi = cc.vv.gameNetMgr.speedTime-Date.now();
            // console.log("daojishi",daojishi)
            var m = Math.floor((daojishi % (1000 * 60 * 60)) / (1000 * 60));
            var s = Math.floor((daojishi % (1000 * 60)) / 1000);
            this._timecount.string  = m+":"+s;
            
        }else{
            this._timecount.string  = "00:00";
            if(cc.vv.gameNetMgr.numOfGames>3&&this._isLingQu){
                alert("获得神秘宝箱!");//弹出领取页面，点击领取后设置为领取状态
                this._isLingQu=false;
            }
        }
    },
