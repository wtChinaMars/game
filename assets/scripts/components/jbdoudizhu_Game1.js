cc.Class({
    ...
    onLoad: function () {
      ...
      this.lblNotice.string = cc.vv.userMgr.notice.msg;
        ...
        this.addComponent("jbdouzizhu_Room1");
    ...
    initEventHandlers:function(){
        ...
        this.node.on('game_beishu',function(data){
            ...
            var beishu1=self.beishu.string;
            if(data.detail.zongbeishu==3){
                self.beishu.string=0;
            }
            if(self.beishu.string==0){
                self.beishu.string=3
            }else{
                self.beishu.string=beishu1*data.detail.zongbeishu; 
            }
           // self.beishu.string=beishu1*data.detail.zongbeishu;
        });
        this.node.on('game_exitroom_push',function(data){
           cc.vv.net.show('exit');
		});
        ...
        this.node.on('game_taskInfo',function(data){
            console.log(data);
            cc.vv.alert.show("提示",data.detail.title+"获得:"+data.detail.prize);
            cc.find("Canvas/game/renwu/info").getComponent(cc.Label).string = data.detail.title;
        });
        this.node.on('game_holds2',function(data){
             self.initpukes();
             self.initOtherpukes();
        });
        this.node.on("game_jb_bz",function(data){
            cc.vv.alert.show("提示",data.detail);
            if(data.detail=="金币不足即将踢出房间!"){
                cc.vv.net.send("exit");
            }
        });
        ...
        this.node.on('game_say_notify',function(data){
            ...
            if(data.detail.pai==null){
              ...
              var index=  cc.vv.gameNetMgr.getLocalIndex(seatData.seatindex);
                ...
                var beishu1=this.beishu.string;
                this.beishu.string=beishu1*2;
    ...
    onGameBegin:function(){
        ...
        //this.beishu.string=2;
        console.log(cc.vv.gameNetMgr.conf,"cc.vv.gameNetMgr.conf");
       ...
       // this.clear_allcard();
        ...
        var qianshu=0;
        if(cc.vv.gameNetMgr.conf==1){
            qianshu=50;
        }else if(cc.vv.gameNetMgr.conf==2){
            qianshu=200;
        }else if(cc.vv.gameNetMgr.conf==3){
            qianshu=800;
        }else if(cc.vv.gameNetMgr.conf==4){
            qianshu=3000;
        }
        this.jishu.string=qianshu;
    },
    setbeishu:function(){
        for (var key in cc.vv.gameNetMgr.seats) {
            var sides = ["myself","right","left"];
    ...
    initpukes:function(){
       ...
       // cc.find("Canvas/game/renwu/info").getComponent(cc.Label).string = cc.vv.gameNetMgr.seats[cc.vv.gameNetMgr.seatIndex].renwu_name;
    ...
    reday:function(){
        for(var i=0;i<cc.vv.gameNetMgr.seats.length;i++){
            if(cc.vv.gameNetMgr.seats[i].userid==cc.vv.userMgr.userid){
                var type = cc.vv.gameNetMgr.conf.room_type;
                if(type==1){
                    if(cc.vv.gameNetMgr.seats[i].coins<500){
                        cc.vv.alert.show("提示","金币不足！",function(){
                            this.jiesan();
                        },true);
                    }
                    return;
                }else if(type==2){
                    if(cc.vv.gameNetMgr.seats[i].coins<2000){
                        cc.vv.alert.show("提示","金币不足！",function(){
                            this.jiesan();
                        },true);
                    }
                    ...
                    if(cc.vv.gameNetMgr.seats[i].coins<10000){
                        cc.vv.alert.show("提示","金币不足！",function(){
                            this.jiesan();
                        },true);
                    }
                    ...
                    if(cc.vv.gameNetMgr.seats[i].coins<=50000){
                        cc.vv.alert.show("提示","金币不足！",function(){
                            this.jiesan();
       ...
       // cc.vv.net.send('ready');
        ...
        if(this._isGameEnd){
            ...
            if(cc.vv.gameNetMgr.seats[cc.vv.gameNetMgr.seatIndex].ready!=true){
                cc.vv.net.send('ready');
            }
    ...
    showChupai:function(seatIndex){
        ...
        var sides = ["myself","right","left"];
            ...
            var qianshu=0;
            if(cc.vv.gameNetMgr.conf.room_type==1){
                qianshu=50;
            }else if(cc.vv.gameNetMgr.conf.room_type==2){
                qianshu=200;
            }else if(cc.vv.gameNetMgr.conf.room_type==3){
                qianshu=800;
            }else if(cc.vv.gameNetMgr.conf.room_type==4){
                qianshu=3000;
            }
            this.jishu.string=qianshu;
