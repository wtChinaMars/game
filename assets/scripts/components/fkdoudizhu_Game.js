cc.Class({
    ...
    onLoad: function () {
        ...
        this.addComponent("fkdouzizhu_Room");
    ...
    initEventHandlers:function(){
        ...
        //   var myselfChild = gameChild.getChildByName("myself");
        //   var myholds = myselfChild.getChildByName("holds");
        //   var otherChild = gameChild.getChildByName("right");
        //   var otherholds = otherChild.getChildByName("holds");
        //   myholds.removeAllChildren();
        //   otherholds.removeAllChildren();
          if(cc.vv.gameNetMgr.seatIndex==0){
            cc.vv.gameNetMgr.seats[0].holds=[];
            cc.vv.gameNetMgr.seats[1].holds=[];
            cc.vv.gameNetMgr.seats[0].folds[0]=11;
            cc.vv.gameNetMgr.seats[1].folds[0]=11;
              while(index<34){
          ...
          var left=-480;
          for(var i=0;i<3;i++){
            var _puke= cc.instantiate(self.card_other);
          ...
          for(var i=fapai.length-1;i>=0;i--){
            var puke= cc.instantiate(self.card_other);
          ...
          var plist=fapaipanel.getChildren();
             ...
             var callback = function () {
                 if (ii== 34)
                 {
                    ...
                    self.initpukes();
                   // self._fapai.active=false;
                    self.unschedule(callback);
                    //  var fapaipanel=cc.find("Canvas/game/fapai");
                    //      fapaipanel.removeAllChildren();
                    plist[0].runAction(cc.spawn(cc.moveBy(0.2, 350, 0),cc.moveBy(0.2, 0, 120)));
                    plist[1].runAction(cc.spawn(cc.moveBy(0.2, 450, 0),cc.moveBy(0.2, 0, 120)));
                    plist[2].runAction(cc.spawn(cc.moveBy(0.2, 550, 0),cc.moveBy(0.2, 0, 120)));
                    ...
                    self.fapai(fapai,ii);
                    if(ii%2==0){
                        plist[36-ii].runAction(cc.sequence(cc.moveBy(0.05, m_x, -150),cc.hide()));
                        m_x+=12;
                    }else{
                        plist[36-ii].runAction(cc.sequence(cc.moveBy(0.05, up_x, 240),cc.hide()));
                        up_x+=35;
                    }
...
.close();
        ...
        this.node.on('hide_readyBtn',function(data){
          //  alert(data.detail.seatindex);
            ...
            if(cc.vv.gameNetMgr.seatIndex==data.detail.seatindex){
                ...
                // var btn2=cc.find("Canvas/btnReady");
                // btn2.active=false;
        ...
        this.node.on('game_over',function(data){
           ...
           if(data.length>0){
                ...
                var shengpai=self.gameRoot.getChildByName("shengyu");
                shengpai.removeAllChildren();
                for(var i=0;i<data[0].shengyu.length;i++){
                 var puke= cc.instantiate(self.card_other);

                     puke.getComponent("card").init(data[0].shengyu[i].val,data[0].shengyu[i].suit,0,"noback");
                     puke.val=data[0].shengyu[i].val;
                     puke.suit=data[0].shengyu[i].suit;

                 shengpai.addChild(puke);
                }
           }
        });
       ...
       this.node.on('game_turn',function(data){
        ...
        if(data.detail.state=="play"){
            ...
            if(localIndex==0){
                ...
                self._times.runAction(cc.sequence(cc.moveTo(0.2,400,150).easing(cc.easeIn(1.0)),cc.callFunc(function(){
                    self._time = 30;
                    self._alertTime = 3;
                 },this,0)));
            }
		...
		//GPs二人
       this.node.on('game_gps',function(data){
            cc.vv.gameNetMgr.juli="距离对家:"+parseInt(data.detail.juli);
            cc.find("Canvas/userinfo_game/juli").getComponent(cc.Label).string =parseInt(cc.vv.gameNetMgr.juli);
        });
		//提示
    ...
    onGameBegin:function(){
        ...
        this.jishu.string=cc.vv.gameNetMgr.rangpai;
    ...
    clear_allcard:function(){
        ...
        var otherChild = gameChild.getChildByName("right");
        var otherholds = otherChild.getChildByName("holds");
        var otherfolds=otherChild.getChildByName("folds");
        ...
        otherholds.removeAllChildren();
        otherfolds.removeAllChildren();

        var userstate=myselfChild.getChildByName("user_speed_state");
        userstate.active=false;
        var otheruserstate=otherChild.getChildByName("user_speed_state");
        otheruserstate.active=false;

        var shengpai=gameChild.getChildByName("shengyu");
        ...
        for (var key in cc.vv.gameNetMgr.seats) {
            ...
            var zhuang1=seat.getChildByName("zhuang1");
            var zhuang2=seat.getChildByName("zhuang2");
            seat.getChildByName("jiabei").active = false;
    ...
    initOtherpukes:function(){
       ...
       for(var s in seats){
            if(s==cc.vv.gameNetMgr.seatIndex){
                ...
                this.card_other_count.string=holds_count;
	...
	user_ctrl_Clicked:function(event){
      ...
      btn1.getComponent(cc.Button).interactable = false;
   ...
   onbtnc:function(){
        ...
        var sides = ["myself","right"];
            ...
            if(cc.vv.gameNetMgr.gamestate=="play"){
                if(pai&&pai.length>0){
                    ...
                    if(pai==2||pai>7){
                        this.jishu.string=cc.vv.gameNetMgr.rangpai+"张";
                        if(pai>=8&&pai<=10){
                            var tishi=this.gameRoot.getChildByName("rangpaitishi");
                            var str=cc.vv.gameNetMgr.rangpai>0?"地主让牌"+cc.vv.gameNetMgr.rangpai+"张":"地主没让牌";
                            tishi.getChildByName("rangpaitishitxt").getComponent(cc.Label).string="地主让牌"+cc.vv.gameNetMgr.rangpai+"张";
                            cc.vv.audioMgr.playSFX("doudizhu/game/xn.mp3");
                            tishi.active=true;
                            setTimeout(() => {
                                tishi.active=false;
                            }, 1500);
    ...
    showAction:function(data){
        ...
        if(data && (data.tishi || data.buchu || data.chupai|| data.jiaodizhu|| data.bujiao|| data.qiangdizhu|| data.buqiang|| data.bujiabei|| data.mingpai|| data.jiabei||data.rangpai)){
    ...
    onBtnDissolveClicked2:function(){
      //  alert("asd");
    ...
    onBtnExit:function(){
        ...
        if(seats[index].userid==cc.vv.gameNetMgr.conf.creator&&cc.vv.userMgr.gems<50){
            cc.vv.alert.show("返回大厅","返回大厅房间仍会保留，快去邀请大伙来玩吧！",function(){
                cc.vv.wc.show('正在返回游戏大厅');
                cc.vv.gameNetMgr.returngame="fangkadoudizhu";
                cc.director.loadScene("hall");
    ...
    share:function(){
        cc.vv.anysdkMgr.share("豆乐棋牌二人斗地主！" ,"房号:" + cc.vv.gameNetMgr.roomId + " 玩法:" + cc.vv.gameNetMgr.getWanfa());
         cc.vv.audioMgr.playSFX("doudizhu/button/anniu.mp3");
    },
    share2:function(){
