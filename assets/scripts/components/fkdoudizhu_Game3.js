cc.Class({
    ...
    onLoad: function () {
        // if(cc.vv.gameNetMgr.conf.GPS){
        //     //   cc.find("Canvas/userinfo_game/gps").active=true;
        //    cc.vv.anysdkMgr.getGPSLocation();
        //    cc.vv.wc.show('正在获取定位信息！');
        //     this.count1 = 0;
        //     this._selfTimer = null;
        //     var self = this;
        //     this._selfTimer=setInterval(function(){
        //       if(self.count1>10)
        //       {
        //           clearInterval(self._selfTimer);
        //           cc.vv.wc.hide();
        //           cc.vv.alert.show("提示", "定位失败!");
        //       }
        //       else
        //       {
        //           self.count1++;
        ...
        //               var lontitude=lontitudestr[1];
                    //     var gpsData = {
                    //         latitude:123.123123,
                    //         lontitude:456.456456,
                    //   };
                    //  cc.vv.net.send("tdk_GPSinfo1", gpsData);
                     // cc.vv.net.send("tdk_GPSinfo1",gpsData);
                    // cc.vv.net.send("123", gpsData);
        //           }
        //       }
        //   },2000);
      ...
      this.lblNotice.string = cc.vv.userMgr.notice.msg;
        ...
        this.addComponent("fkdouzizhu_Room3");
...
// cc.vv.gameNetMgr.seats[0].lat=38.2884460000,cc.vv.gameNetMgr.seats[0].lot=38.2884460000;
// cc.vv.gameNetMgr.seats[1].lat=38.2884390000,cc.vv.gameNetMgr.seats[1].lot=104.9756100000;
// cc.vv.gameNetMgr.seats[2].lat=38.2883790000,cc.vv.gameNetMgr.seats[2].lot=104.9749010000;
...
// if(cc.vv.gameNetMgr.gps==true){
//     if(cc.vv.gameNetMgr.seats.length==2){
//         for(var i = 0;i<cc.vv.gameNetMgr.seats.length;i++){
//             if(cc.vv.gameNetMgr.seats[i].userid>0){

//                     if(cc.vv.gameNetMgr.seats[j].userid>0){
//                        var juli =  getDisance(cc.vv.gameNetMgr.seats[i].lat,cc.vv.gameNetMgr.seats[i].lot,cc.vv.gameNetMgr.seats[0].lat,cc.vv.gameNetMgr.seats[0].lot);
//                        if(juli<=50){
//                            seats[0].getChildByName("gps1").getChildByName("gpstu").active = true;
//                        }
//                     }
...
//     }else if(cc.vv.gameNetMgr.seats.length==3){
//         for(var i = 0;i<cc.vv.gameNetMgr.seats.length;i++){
//             if(cc.vv.gameNetMgr.seats[i].userid>0){
//                 if(cc.vv.gameNetMgr.seats[i].userid==cc.vv.gameNetMgr.seats[0].userid)continue;
//                 for(var j = 0;j<cc.vv.gameNetMgr.seats.length;j++){
//                     if(cc.vv.gameNetMgr.seats[j].userid>0){
//                         if(cc.vv.gameNetMgr.seats[j].userid==cc.vv.gameNetMgr.seats[0].userid)continue;
//                        var juli =  getDisance(cc.vv.gameNetMgr.seats[i].lat,cc.vv.gameNetMgr.seats[i].lot,cc.vv.gameNetMgr.seats[j].lat,cc.vv.gameNetMgr.seats[j].lot);
//                        if(juli<=50000){
//                            seats[0].getChildByName("gps1").getChildByName("gpstu").active = true;
//                        }
//                     }
    ...
    onBtnExit:function(){
        ...
        if(seats[index].userid==cc.vv.gameNetMgr.conf.creator&&cc.vv.userMgr.gems<50){
            cc.vv.alert.show("返回大厅","返回大厅房间仍会保留，快去邀请大伙来玩吧！",function(){
                ...
                cc.vv.gameNetMgr.returngame="fangkadoudizhu3";
                cc.director.loadScene("hall");
