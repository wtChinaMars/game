...
cc.Class({
    ...
    onLoad: function () {
        ...
        if(btnClose){
            cc.vv.utils.addClickEvent(btnClose,this.node,"GameResult_fk","onBtnCloseClicked");
        }
        
        var btnShare = cc.find("Canvas/game_result/btnShare");
        if(btnShare){
            cc.vv.utils.addClickEvent(btnShare,this.node,"GameResult_fk","onBtnShareClicked");
        }
    ...
    onGameEnd:function(endinfo){
        ...
        var users = this._gameresult.getChildByName("seats").getChildren();
        for(var i = 0; i < seats.length; ++i){
            var seat = seats[i];
            var isBigwin = false;
            if(seat.score > 0){
            ...
            var sss=users[i].getChildByName("icon");
             var id=users[i].getChildByName("id");
             id.getComponent(cc.Label).string=seat.userid;
            var name=users[i].getChildByName("name");
             name.getComponent(cc.Label).string=seat.name;
             var score=users[i].getChildByName("score");
            score.getComponent(cc.Label).string=endinfo[i].score;
             var ss=sss.getComponent("ImageLoader");
             ss.setUserID(seat.userid);
           //  users[i].active=true;
