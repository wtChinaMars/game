...
cc.Class({
    ...
    onLoad: function () {
        ...
        this._leixingxuanze = [];
        cc.find("Canvas/FK/CreateRoom/game_list/fangkadoudizhu/dianganghua")
        // cc.vv.radiogroupmgr.check(cc.find("Canvas/FK/DaiKaiRoom/game_list/fangkadoudizhu/dianganghua/RadioButton/ren2"));
    ...
    //    var gps =  wanfaxuanze.children[3].getComponent('CheckBox').checked;
            ...
            if(cc.vv.userMgr.gems<30){
                cc.vv.alert.show("提示","钥匙必须大于等于30张才能创建房间！");
            }else{
    ...
    createRoom: function (val,val1) {
        var self = this;
        var wanfaxuanze = this._currentGame.getChildByName('wanfaxuanze');
       var gps =  wanfaxuanze.children[3].getComponent('CheckBox').checked;
       var renshu=this.getSelectedOfRadioGroup('dianganghua');
       if(val1!=1){
        if(gps==true&& cc.vv.userMgr.hallgps==false&&renshu==1){
            cc.vv.alert.show("提示","GPS未开启");
            return;
           }
       }
        ...
        if(cc.vv.userMgr.account==undefined){
            var data = {
                account: cc.vv.userMgr.account2,
    ...
    console.log(gps);
     //   var dianganghua = this.getSelectedOfRadioGroup('dianganghua');
        if(jushuxuanze==0&&renshu==2)
        {
            jushuxuanze=15;
        }else if(renshu==2){
            jushuxuanze=30;
        }
        if(jushuxuanze==0&&renshu==3)
        {
            jushuxuanze=12;
        }else if(renshu==3){
            jushuxuanze=30;
