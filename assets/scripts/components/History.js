cc.Class({
    ...
    onLoad: function () {
        this._history = this.node.getChildByName("bottom_left").getChildByName("fk_bottom").getChildByName("zhanji").getChildByName("history");
        ...
        var node = cc.find("Canvas/FK/bottom_left/fk_bottom/btnzhanji");        
        this.addClickEvent(node,this.node,"History","onBtnHistoryClicked");
        
        var node = cc.find("Canvas/FK/bottom_left/fk_bottom/zhanji/history/btn_back");  
    ...
    initRoomHistoryList:function(data){
        for(var i = 0; i < data.length; ++i){
            ...
            node.getChildByName("rooinfo").getComponent(cc.Label).string = "斗地主(" + data[i].renshu+" 人"+data[i].jushu+"局)";
            ...
            for(var j = 0; j < data[i].seats.length; ++j){
                var s = data[i].seats[j];
                sum++;
                var ren1=node.getChildByName("player"+j);
                ren1.active = true;
                var ren=node.getChildByName("player"+j).getChildByName("score");
                
                if(s.score>0){
                    ren.getComponent(cc.RichText).string ='<color=#1DB400>+'+s.score+'</c>'
                    ren.Color='#1DB400';

                }else{
                    ren.getComponent(cc.RichText).string ='<color=#FF0000>'+s.score+'</c>'
                   
                    ren.Color='#FF0000';
                }
               // ren1.getChildByName("head")
                //s.userid

                if(s.userid>0){
                    var sss=  ren1.getChildByName("head");
                    var head=sss.getComponent("ImageLoader");
                    head.setUserID(s.userid);
                  }
                //console.log(info);
                // if(s.userid==cc.vv.userMgr.userId){
                //     var info ="+" +  s.score;
                //     node.getChildByName("info0").getComponent(cc.Label).string = info;
                // }
