...
cc.Class({
    ...
    init:function(data){
        ...
        this._currentIndex = 0;
        this._lastAction = null;
		cc.vv.gameNetMgr.gamestate="play";
		cc.vv.gameNetMgr.dissoveData=null;
    ...
    getNextAction:function(){
		...
		var kind = this._actionRecords[this._currentIndex++];
		var beishu = this._actionRecords[this._currentIndex++];
		var rangpai = this._actionRecords[this._currentIndex++];
		var jishu = this._actionRecords[this._currentIndex++];
        return {si:si,type:action,pai:pai,kind:kind,beishu:beishu,rangpai:rangpai,jishu:jishu};
    },
    ...
    takeAction:function(){
        ...
        if(action.type == ACTION_CHUPAI){
            //console.log("chupai");
            cc.vv.gameNetMgr.doChupai(action.si,action.pai,action.kind,action.beishu,action.rangpai,action.jishu);
            return 1.0;
        }
